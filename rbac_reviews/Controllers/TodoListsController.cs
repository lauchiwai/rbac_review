...
public class TodosController : ControllerBase
{
    ...
    [HttpGet("get-all")]
    public async Task<ActionResult<ResultDto<List<TodoViewModel>>>> GetAllTodos([FromQuery] int CurrentUserId)
    {
        if (CurrentUserId <= 0)
        {
            return BadRequest(ResultDto<List<TodoViewModel>>.BadRequest("Invalid CurrentUserId"));
        }

        var request = new GetTodosRequest { CurrentUserId = CurrentUserId };
    ...
    {
        if (id <= 0 || request?.CurrentUserId <= 0)
        {
            return BadRequest(ResultDto<TodoViewModel>.BadRequest("Invalid ID or CurrentUserRoleId"));
        }
