...
public class TodoReviewsController : ControllerBase
{
    ...
    public async Task<ActionResult<ResultDto<List<TodoViewModel>>>> GetReviewTodos([FromQuery] int CurrentUserId, [FromQuery] string? status, [FromQuery] int? reviewLevel)
    {
        if (CurrentUserId <= 0)
        {
