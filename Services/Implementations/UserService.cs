...
namespace Services.Implementations
{
    public class UserService : IUserService
    {
        ...
        public async Task<ResultDto<RoleUsersResponse>> GetUsersByRoleAsync(int roleId)
        {
            try
            {
                var role = await _roleRepository.GetByIdAsync(roleId);
                if (role == null)
                    return ResultDto<RoleUsersResponse>.Failure($"Role with ID {roleId} not found");

                Expression<Func<Users_Roles, bool>> predicate = ur => ur.RoleId == roleId;
                var userRoles = await _userRoleRepository.FindAsync(predicate);
                ...
                return ResultDto<RoleUsersResponse>.Success(response);
            }
            catch (Exception ex)
            {
                return ResultDto<RoleUsersResponse>.Failure($"Error retrieving users by role: {ex.Message}");
            }
        }

        public async Task<ResultDto<bool>> HasRoleAsync(int userId, int roleId)
        {
            try
            {
                var user = await _userRepository.GetByIdAsync(userId);
                if (user == null)
                    return ResultDto<bool>.Failure($"User with ID {userId} not found");

                var role = await _roleRepository.GetByIdAsync(roleId);
                if (role == null)
                    return ResultDto<bool>.Failure($"Role with ID {roleId} not found");

                Expression<Func<Users_Roles, bool>> predicate = ur =>
                    ur.UserId == userId && ur.RoleId == roleId;

                var userRoles = await _userRoleRepository.FindAsync(predicate);
                var hasRole = userRoles.Any();

                return ResultDto<bool>.Success(hasRole);
            }
            catch (Exception ex)
            {
                return ResultDto<bool>.Failure($"Error checking role: {ex.Message}");
            }
        }
    }
}